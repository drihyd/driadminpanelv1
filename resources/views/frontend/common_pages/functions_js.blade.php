...
function update_cart(itemid,qty,unitprice,sku_id){
	...
	$("#ajaxfilter").change(function(e) {
			...
			if(filters['my_range'] !== undefined) {
				my_range =  filters['my_range'].join(',');
				temp['my_range'] = my_range;
		...
		$("#products-area").html("<h4 class='text-center mt-5 mb-5'>Fetching data.</h4>.");
...
<script>
 $(document).ready(function () {
  $(".addtocartsubmitButton").click(function(event){
      ...
      let price_break_up = $("input[name=price_break_up]").val();
...
<script type="text/javascript">
   $(document).ready(function(){ 
     ...
     if ($('#check-address').is(":checked")) {
      ...
      $('#billing_log-email').val($('#log-email').val());
