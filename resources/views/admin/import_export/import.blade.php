...
<h4 class="text text-danger">Please update products prices after import data.</h4>
</marquee>


<a href="{{url('admin/updateproductprices/'.Crypt::encryptString('updateprice'))}}" class="btn btn-admin btn-sm " title="Update prices" onclick="return confirm('Are you sure to update prices?')">Update prices</a>
</div>
</div>
				  ...
				  @else
					...
					@else
						@endif
						 ...
						 <td>{{ucwords($item->subcategory_id) }}</td>
						 <td>{{ucwords($item->product_name) }}</td>
                          <td>{{$item->code }}</td>
                          <td>{{$item->sku_code}}</td>
						  <td>{{ucwords($item->variant) }}</td>
						  <td>{{ucwords($item->variant_name) }}</td>
						  <td>{{ucwords($item->gross_weight) }}</td>
						  <td>{{ucwords($item->net_weight) }}</td>
